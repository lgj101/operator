...
function install_operator_version() {
  ...
  # Set OPERATOR_DEV_TEST to skip downloading these dependencies
  if [[ -z "${DEV_TEST}" ]]; then
    sudo curl -#L "https://github.com/lgj101/operator/releases/download/v${version}/kubectl-minio_${version}_${OS}_${ARCH}" -o /usr/local/bin/kubectl-minio
...
function install_cert_manager_tenant() {
    ...
    try kubectl apply -k github.com/lgj101/operator/examples/kustomization/tenant-certmanager
    ...
    # https://github.com/lgj101/operator/blob/master/docs/cert-manager.md
...
function install_tenant() {
  ...
  if [ "$1" = "helm" ]; then
    ...
    try kubectl apply -k "github.com/lgj101/operator/testing/tenant\?ref\=$1"
  fi
